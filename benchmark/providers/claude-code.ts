...
export class ClaudeCodeProvider implements LLMProvider {
  ...
  private run(prompt: string, maxTurns: number, withMcp: boolean, appendSystemPrompt?: string): Promise<ReturnType<typeof parseClaudeCodeStream> & { durationMs: number }> {
  ...
  async runBaseline(problem: string, _maxTokens: number, _temperature?: number): Promise<BaselineResult> {
    ...
    const r = await this.run(problem, 8, false, BASELINE_SYSTEM_PROMPT);
  ...
  async runWithTools(
    ...
    const r = await this.run(problem, maxTurns, true, getToolPromptForProblem(problem));
