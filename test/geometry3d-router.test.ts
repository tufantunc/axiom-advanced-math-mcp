...
describe('Router — geometry3d', () => {
  it.each([
    ['distance3d()', 'distance3d([0,0,0], [1,2,2])', 'distance3d'],
    ['cross()', 'cross([1,0,0], [0,1,0])', 'cross'],
    ['vector_norm()', 'vector_norm([2,3,6])', 'vector_norm'],
    ['plane_from_points()', 'plane_from_points([0,0,0],[1,0,0],[0,1,0])', 'plane_from_points'],
  ])('routes %s to geometry3d', (_label, input, operation) => {
    const r = route(input);
    expect(r.handler).toBe('geometry3d');
    expect(r.args.operation).toBe(operation);
  });

  it('parses the distance3d argument lists', () => {
    const r = route('distance3d([0,0,0], [1,2,2])');
    expect(r.args.lists).toEqual([
      [0, 0, 0],
      [1, 2, 2],
    ]);
  });
