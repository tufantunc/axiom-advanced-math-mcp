...
export const geometrySchema = z.object({
  operation: z
    ...
    .describe(
      'Geometry operation:\n' +
        '  distance — distance between two points (x1,y1) and (x2,y2)\n' +
        '  midpoint — midpoint of segment between two points\n' +
        '  slope — slope of line through two points\n' +
        '  area_triangle — area of triangle from 3 points or base+height\n' +
        '  area_polygon — area of polygon from ordered vertices (shoelace)\n' +
        '  area_circle — area from radius or diameter\n' +
        '  perimeter_polygon — perimeter of polygon from vertices\n' +
        '  circumference — circumference from radius or diameter\n' +
        '  line_intersection — intersection of two lines (a1,b1,c1) and (a2,b2,c2) where a*x+b*y+c=0\n' +
        '  point_line_distance — distance from point (px,py) to line a*x+b*y+c=0\n' +
        '  angle_between_lines — angle in degrees between two lines'
    ),
  points: z
    .array(z.tuple([z.number(), z.number()]))
    .optional()
    .describe(
      'Array of (x,y) coordinate pairs. Most operations need 2 points, triangle needs 3, polygon needs ordered vertices.'
    ),
  radius: z.number().optional().describe('Radius (for circle operations)'),
  diameter: z
    .number()
    .optional()
    .describe('Diameter (alternative to radius for circle operations)'),
  base: z.number().optional().describe('Base length (for triangle area with height)'),
  height: z.number().optional().describe('Height (for triangle area with base)'),
  line1: z
    .tuple([z.number(), z.number(), z.number()])
    .optional()
    .describe('First line as [a, b, c] where ax + by + c = 0'),
  line2: z
    .tuple([z.number(), z.number(), z.number()])
    .optional()
    .describe('Second line as [a, b, c] where ax + by + c = 0'),
});
