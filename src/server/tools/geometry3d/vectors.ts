    ...
    switch (op) {
      case 'distance3d': {
        ...
        return formatToolResponse({
          ...
          notes: [`Distance between ${vfmt(p)} and ${vfmt(q)}`],
        });
      }
      case 'midpoint3d': {
        const p = need(lists[0], 3, 'first point');
        ...
        return formatToolResponse({
          result: vfmt(m),
          notes: [`Midpoint of ${vfmt(p)} and ${vfmt(q)}`],
        });
      }
      case 'dot': {
      ...
      case 'vector_norm': {
        ...
        return formatToolResponse({
          result: formatNumber(vnorm(v)),
          notes: [`Magnitude of ${vfmt(v)}`],
        });
      }
      case 'angle_vectors': {
        const u = need(lists[0], 3, 'first vector');
        ...
        if (isZero(nu) || isZero(nv))
          return formatErrorResponse('angle_vectors: vectors must be non-zero');
        const cos = Math.max(-1, Math.min(1, vdot(u, v) / (nu * nv)));
        const deg = (Math.acos(cos) * 180) / Math.PI;
        return formatToolResponse({
          result: `${formatNumber(deg)}°`,
          notes: [`Angle between ${vfmt(u)} and ${vfmt(v)}`],
        });
      }
      default:
