...
export function rewriteCombinatorics(expr: string): string {
  return expr
    .replace(/(?<![A-Za-z])C\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)/g, 'comb($1,$2)')
    .replace(/(?<![A-Za-z])P\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)/g, 'perm($1,$2)')
    .replace(/\b(?:combinations|nCr|binomial)\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)/gi, 'comb($1,$2)')
    .replace(/\b(?:permutations|nPr)\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)/gi, 'perm($1,$2)');
}
